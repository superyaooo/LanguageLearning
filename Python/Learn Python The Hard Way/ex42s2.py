...
class Engine(object):
    def __init__(self, m, start):
        self.start = start
        self.m = m         # sets variable m. (m --> Map())
        
    def play(self): 

        next = self.start  # assigns start to next
        
        while True:
            print "\n-----------"
            room = getattr(self.m, next)    # gets the attribute of next in m (Map(), gets whatever is returned)
            next = room()
            

a_map = Map()  # assigns Map() to a_map
a_game = Engine(a_map, 'princess_lives_here') # a_map is m, 'princess_lives_here' is start
