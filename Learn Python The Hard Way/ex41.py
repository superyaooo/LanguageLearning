...
def death():
    quips = ["You died. You kinda suck at this.", 
            ...
            "Such a luser.", 
            "I have a small puppy that's better at this."]   # a list of death reasons

    print quips[randint(0, len(quips)-1)]  # prints a random element from the list, quips[0]~quips[3]
    exit(1) # means the program ends/exits due to an error.
...
def princess_lives_here():
    ...
    if eat_it == "eat it":
        ...
        return 'death'   # returns the key 'death' for the value(the actual function)
        ...
        return 'gold_koi_pond'  # returns to function 'gold_koi_pond'
...
}  # a dictionary of functions, first element is the key, second is the value(actual function) 
...
def runner(map, start):
    ...
    while True:
        room = map[next] # assigns the value of the key 'next' from the dictionary
        print "\n-------------"
        next = room()

runner(ROOMS, 'princess_lives_here') # calls function 'runner', starts game with the key for princess function
